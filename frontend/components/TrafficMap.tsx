...
export function TrafficMap({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Base tiles -- TomTom basic dark style if key, else free OSM
      if (TOMTOM_KEY) {
        L.tileLayer(
          `https://api.tomtom.com/map/1/tile/basic/night/{z}/{x}/{y}.png?key=${TOMTOM_KEY}`,
          { maxZoom: 22, attribution: '&copy; <a href="https://www.tomtom.com">TomTom</a>' },
        ).addTo(map);
      } else {
      ...
      if (TOMTOM_KEY) {
        const flow = L.tileLayer(
          `https://api.tomtom.com/traffic/map/4/tile/flow/relative-dark/{z}/{x}/{y}.png?key=${TOMTOM_KEY}`,
      ...
      try {
        const segs = (await fetch("/api/segments", { cache: "no-store" })
          .then((r) => (r.ok ? r.json() : []))) as Segment[];
        drawSegments(segs);
  ...
  const drawSegments = useCallback(async (segs: Segment[]) => {
    ...
    segs.forEach((s) => {
      ...
      const stroke = "rgba(232,132,60,0.85)";
      const halo = L.polyline(coords, { color: "#000", opacity: 0.55, weight: 8, lineCap: "round" });
      const line = L.polyline(coords, {
        color: stroke, opacity: 0.0, weight: 4, lineCap: "round",
      }).bindTooltip(
        `<div style="font-size:11px"><strong>${escapeHtml(s.name)}</strong><br/>${s.segment_type}</div>`,
        { sticky: true },
      );
  ...
  return (
    <div className="map-wrap" style={{ height }}>
      ...
      {/* Zoom controls */}
      {showControls && (
        <div className="map-ctrl" style={{ right: 12, bottom: 12, flexDirection: "column" }}>
